...
import sys, os, time, survival_algorithms, common_algorithms, attack_algorithms
...
floodfill = common_algorithms.Floodfill()
optimizeSpaceAlgorithm = survival_algorithms.OptimizeSpaceAlgorithm()
minimax = attack_algorithms.Minimax()
aStar = attack_algorithms.AStar()
dijkstra = common_algorithms.Dijkstra()
...
class Board(object):
    ...
    def __init__(self, width, height, board):
        ...
        self._me = None
        self._them = None
        #if len(floodfill.avgTime) > 0:
	    #    log("FLOODFILLTIME: " + str(float(sum(floodfill.avgTime)) / len(floodfill.avgTime)))
        #floodfill.avgTime = []
    ...
    def impassableNodes(self):
    	impassableNodes = []
    	for y in range(self.height):
    		for x in range(self.width):
    			if not self.passable((y, x)):
    				impassableNodes.append((y, x))
    	return impassableNodes
        
    def distance(self, start, goal):
    	yDistance = abs(start[1] - goal[1])
    	xDistance = abs(start[0] - goal[0])
    	return float((yDistance * yDistance + xDistance * xDistance)**.5)
    ...
    def adjacentImpassable(self, origin):
    	return [adj for adj in self.adjacent(origin) if not self.passable(adj)]
    	
    def adjacentImpassableOrVisited(self, origin, visited):
    	return [adj for adj in self.adjacent(origin) if not self.passable(adj) or adj in visited]

    def moveableDestinations(self, origin, exclude=[]):
        return [self.rel(dir, origin) for dir in self.moves(origin) if self.rel(dir, origin) not in exclude]
	
    def moves(self):
        return moves(self.me())
		
    def moves(self, origin=None):
